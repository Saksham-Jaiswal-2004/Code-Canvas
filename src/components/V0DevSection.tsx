    ...
    <section id="v0dev" className="py-24 px-4">
      <div className="container mx-auto max-w-5xl">
        ...
        <AnimatedSection delay={0.1}>
          <div className="glass rounded-xl p-6 border-primary/30 neon-glow mb-16 flex items-start gap-4">
        ...
        <div className="space-y-3 mb-8">
          {steps.map((step, i) => (
            <AnimatedSection key={i} delay={0.1 + i * 0.05}>
              <div className="glass rounded-lg p-4 flex items-center gap-4 group hover:border-primary/30 transition-all">
                <div className="w-8 h-8 rounded-full bg-primary/20 text-primary flex items-center justify-center font-display font-bold text-sm shrink-0">
                  {i + 1}
                </div>
                <p className="text-sm text-foreground">{step}</p>
                {i < steps.length - 1 && (
                  <ArrowRight size={14} className="text-muted-foreground ml-auto hidden sm:block" />
                )}
                {i === steps.length - 1 && (
